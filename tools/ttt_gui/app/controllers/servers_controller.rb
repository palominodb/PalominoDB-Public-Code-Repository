class ServersController < ApplicationController
  def index
    ...
    @servers.each do |s|
      ...
      update_graph(s, "Server Aggregate - #{s}", :summary)
    end
  end

  def show
    @server=Server.find(params[:id])
    update_graph(@server.name, "Server Aggregate - #{@server.name}")
  end

  private
  def update_graph(server_name, title, type=:full)
    rrd_path=TTT_CONFIG['formatter_options']['rrd']['path']+"/#{server_name}/server_#{server_name}.rrd"
    rrd_bin=TTT_CONFIG['formatter_options']['rrd']['bin']
    rrd_defs=[]
    rrd_vdefs=[]
    opts = [
      "graph",
      "#{RAILS_ROOT}/public/images/graphs/server_#{server_name}_#{@since_string}.png",
